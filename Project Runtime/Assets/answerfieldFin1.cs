...
public class answerfieldFin1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (inputField == null)
        {
            Debug.LogWarning("answerfieldFin1: No InputField assigned or found on " + gameObject.name);
            return;
        }
    ...
    public void OnSubmit(string text)
    {
        ...
        if (text.Trim().Equals(correctAnswer, System.StringComparison.OrdinalIgnoreCase))
        {
            ...
            if (hasAnswered)
            {
                Debug.Log("answerfieldFin1: Already answered; skipping timeline replay.");
            ...
            Debug.Log("answerfieldFin1: Correct answer entered. Revealing platforms and triggering timelines.");
            ...
            {
                Debug.LogWarning("answerfieldFin1: No smallplatscrpt16 instances found in the scene.");
            }
            ...
            if (timelineControllers != null && timelineControllers.Length > 0)
            {
                foreach (var ctl in timelineControllers)
                {
                    ...
                    {
                        ctl.TriggerTimeline();
                        Debug.Log($"answerfieldFin1: Triggered timeline controller '{ctl.name}'.");
                    }
                }
            }
            else
            {
                ...
                if (directors != null && directors.Length > 0)
                {
                    ...
                    {
                        pd.Play();
                        Debug.Log($"answerfieldFin1: Fallback played PlayableDirector '{pd.name}'.");
                    }
                }
                else
                {
                    Debug.LogWarning("answerfieldFin1: No timeline controllers or PlayableDirectors found to trigger.");
                }
            }
        ...
        {
            Debug.Log($"answerfieldFin1: Incorrect answer entered: '{text}'");
